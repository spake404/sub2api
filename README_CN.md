...
基于 Sub2API **0.1.178** 的公开源码，包含 OpenAI OAuth HTTP Responses
可选的**子代理会话 v1 / v2**。模式说明见 [Summary.md](Summary.md)，
本地构建说明见 [DEV_GUIDE.md](DEV_GUIDE.md)。

请从本仓库源码构建以使用这些功能。下方原版文档中的上游安装链接和镜像不包含本分支改动。
本仓库不包含线上账号配置、服务器备份或内部部署历史。
公开版不内置 Google OAuth 客户端凭据，需按开发指南自行配置；OpenAI 子代理模式不受影响。
