...
describe('BulkEditAccountModal', () => {
  it.each(['subagent', 'subagent_v2'])('explicitly enables and saves %s for OAuth accounts', async (mode) => {
    ...
    await wrapper.get('[data-testid="bulk-codex-fingerprint-mode-select"]').setValue(mode)
    ...
    expect(adminAPI.accounts.bulkUpdate).toHaveBeenCalledWith([1, 2], expect.objectContaining({
      extra: expect.objectContaining({ codex_fingerprint_mode: mode })
    }))
  })
