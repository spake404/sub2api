...
describe('CreateAccountModal OpenAI long-context billing', () => {
  ...
  it.each(['subagent', 'subagent_v2'])('includes %s when explicitly selected during OAuth import', async (mode) => {
    const wrapper = mountModal()
    await selectButtonByText(wrapper, 'OpenAI')
    const select = wrapper.findAllComponents({ name: 'Select' }).find((component) =>
      component.attributes('data-testid') === 'create-codex-fingerprint-mode-select'
    )
    ...
    await wrapper.get('form#create-account-form input[type="text"]').setValue('Subagent mock account')
    await wrapper.get('form#create-account-form').trigger('submit.prevent')
