...
describe('EditAccountModal', () => {
  ...
  it.each(['subagent', 'subagent_v2'])('loads, saves and disables %s on an OAuth account', async (mode) => {
    const account = buildAccount()
    account.type = 'oauth'
    account.extra = { codex_fingerprint_mode: mode }
    account.credentials = { access_token: 'mock-token' }
    checkMixedChannelRiskMock.mockResolvedValue({ has_risk: false })
    updateAccountMock.mockResolvedValue(account)
    const wrapper = mountModal(account)
    const select = wrapper.get('[data-testid="edit-codex-fingerprint-mode-select"]')
    expect((select.element as HTMLSelectElement).value).toBe(mode)
    expect(wrapper.text()).toContain(mode === 'subagent_v2' ? 'admin.accounts.openai.codexFingerprintSubagentV2Desc' : 'admin.accounts.openai.codexFingerprintSubagentDesc')
    await wrapper.get('form#edit-account-form').trigger('submit.prevent')
    expect(updateAccountMock.mock.calls.at(-1)?.[1]?.extra?.codex_fingerprint_mode).toBe(mode)
    await select.setValue('off')
    await wrapper.get('form#edit-account-form').trigger('submit.prevent')
    expect(updateAccountMock.mock.calls.at(-1)?.[1]?.extra).not.toHaveProperty('codex_fingerprint_mode')
  })
