...
Public source based on Sub2API **0.1.178**, including selectable **subagent v1 / v2**
modes for OpenAI OAuth HTTP Responses. See [Summary.md](Summary.md) for the mode
behavior and [DEV_GUIDE.md](DEV_GUIDE.md) for building this fork.

Build this repository from source to use these modes. The upstream download
links and images in the original documentation below do not contain this fork's
changes. No live account configuration or private deployment history is included.
Google OAuth client credentials must be supplied through local configuration;
see the development guide. OpenAI subagent modes are unchanged.
